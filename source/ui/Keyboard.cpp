...
void VirtualKeyboard::open(const std::string& prompt) {
    m_open = true;
    m_text.clear();
    m_prompt = prompt;
    m_row = 0;
...
void VirtualKeyboard::render(SDL_Renderer* renderer, TTF_Font* font) {
    ...
    if (!m_prompt.empty()) {
        SDL_Color promptColor{37, 211, 102, 255};
        SDL_Surface* surf = TTF_RenderUTF8_Blended(font, m_prompt.c_str(), promptColor);
        if (surf) {
            SDL_Texture* tex = SDL_CreateTextureFromSurface(renderer, surf);
            if (tex) {
                SDL_Rect dst{20, panel.y - surf->h - 12, surf->w, surf->h};
